namespace Aeco.Renderer.GL;

using System.Collections.Immutable;

public struct Children : IGLObject
{
    public ImmutableArray<Guid> Ids { get; internal set; }
        = ImmutableArray<Guid>.Empty;

    public Children() {}

    public void Dispose() => this = new();

    public static void Recurse(IDataLayer<IComponent> context, Guid id, Action<IDataLayer<IComponent>, Guid> action)
    {
        action(context, id);
        RecurseChildren(context, id, action);
    }

    public static void RecurseChildren(IDataLayer<IComponent> context, Guid id, Action<IDataLayer<IComponent>, Guid> action)
    {
        if (!context.TryGet<Children>(id, out var children)) {
            return;
        }
        var childrenIds = children.Ids;
        if (childrenIds.Length > 64) {
            void DoRecurse(Guid childId) => Recurse(context, childId, action);
            childrenIds.AsParallel().ForAll(DoRecurse);
        }
        else {
            for (int i = 0; i != childrenIds.Length; ++i) {
                var childId = childrenIds[i];
                action(context, childId);
                RecurseChildren(context, childId, action);
            }
        }
    }

    public static void Recurse<T>(
        IDataLayer<IComponent> context, Guid id, T initial, Func<IDataLayer<IComponent>, Guid, T, T> transformer)
        => RecurseChildren(context, id, transformer(context, id, initial), transformer);

    public static void RecurseChildren<T>(
        IDataLayer<IComponent> context, Guid id, T initial, Func<IDataLayer<IComponent>, Guid, T, T> transformer)
    {
        if (!context.TryGet<Children>(id, out var children)) {
            return;
        }
        var childrenIds = children.Ids;
        if (childrenIds.Length > 64) {
            void DoRecurse(Guid childId) 
                => RecurseChildren(context, childId, transformer(context, childId, initial), transformer);
            childrenIds.AsParallel().ForAll(DoRecurse);
        }
        else {
            for (int i = 0; i != childrenIds.Length; ++i) {
                var childId = childrenIds[i];
                var res = transformer(context, childId, initial);
                RecurseChildren(context, childId, res, transformer);
            }
        }
    }

    public static void RecurseTerminabe<T>(
        IDataLayer<IComponent> context, Guid id, T initial, Func<IDataLayer<IComponent>, Guid, T, (bool, T)> transformer)
    {
        var (cont, res) = transformer(context, id, initial);
        if (cont) {
            RecurseChildrenTerminable(context, id, res, transformer);
        }
    }

    public static void RecurseChildrenTerminable<T>(
        IDataLayer<IComponent> context, Guid id, T initial, Func<IDataLayer<IComponent>, Guid, T, (bool, T)> transformer)
    {
        if (!context.TryGet<Children>(id, out var children)) {
            return;
        }
        var childrenIds = children.Ids;
        if (childrenIds.Length > 64) {
            void DoRecurse(Guid childId) {
                var (cont, res) = transformer(context, childId, initial);
                if (cont) {
                    RecurseChildrenTerminable(context, childId, res, transformer);
                }
            }
            childrenIds.AsParallel().ForAll(DoRecurse);
        }
        else {
            for (int i = 0; i != childrenIds.Length; ++i) {
                var childId = childrenIds[i];
                var (cont, res) = transformer(context, childId, initial);
                if (cont) {
                    RecurseChildrenTerminable(context, childId, res, transformer);
                }
            }
        }
    }
}