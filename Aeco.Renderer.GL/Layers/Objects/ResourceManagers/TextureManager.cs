...
public class TextureManager : ResourceManagerBase<Texture, TextureData, TextureResource>
{
    protected override void Initialize(
        IDataLayer<IComponent> context, Guid id, ref Texture texture, ref TextureData data, bool updating)
    {
        if (updating) {
    ...
    protected override void Uninitialize(IDataLayer<IComponent> context, Guid id, in Texture texture, in TextureData data)
    {
        GL.DeleteTexture(data.Handle);
    }
}